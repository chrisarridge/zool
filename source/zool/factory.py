...
def vertical_stack(heights, labels=None, **kwargs):
	...
	"""
	fig = zool.Figure(figheight=zool.core.FromChildren(),
						layout='vertical', **kwargs)
	for i in range(len(heights)):
		if labels is None:
			fig.add('base', width=zool.core.FromParent(),
					height=zool.core.Fixed(heights[i]), label=str(i))
		else:
			fig.add('base', width=zool.core.FromParent(),
					height=zool.core.Fixed(heights[i]), label=labels[i])
...
def triangle(vars, d2d=4.0, d1d=1.0, m_padding=0.2, t_padding=0.1, **kwargs):
	...
	Additional keyword arguments are passed straight through to the Figure class
	constructor.
	...
	fig = zool.Figure(figwidth=zool.core.FromChildren(),
	...
	fig.add('base', width=zool.core.Fixed(d1d),
	...
	fig.add('tleft-frame', width=zool.core.FromParent(),
...
#				label='{:1d}-top-padding'.format(i+1))
	...
	for i in range(n-1):
		...
		for j in range(i):
			fig.add(frame, width=zool.core.FromParent(),
				height=zool.Fixed(d2d))
...
def triangle_equal(vars, figwidth, d1d=1.0, m_padding=0.2, t_padding=0.1, **kwargs):
	"""This function returns a triangle plot of panels.
	...
	fig = zool.Figure(figwidth=figwidth,
						figheight=zool.core.Named('tleft-frame'),
						layout='horizontal', padding=m_padding, **kwargs)
	n = len(vars)
	...
	fig.add('base', width=zool.core.Fixed(d1d),
				...
				label='tleft-frame')
	for i in range(n-1):
		fig.add('base', width=zool.core.Fill(),
				height=zool.core.FromChildren(),
				layout='vertical', padding=t_padding,
				label='t{:1d}-frame'.format(i+1))
...
def subplot(nrows, ncolumns, width, fixed_height=None, fixed_aspect=None, label_fmt='r{:02d}c{:02d}', padding=0.25, **kwargs):
	"""This function returns a simple grid plot of panels.

	Additional keyword arguments are passed straight through to the Figure class
	constructor.  This function can only accept either fixed_height or a
	fixed_aspect specification, both or neither will generate an exception.
	...
	if (fixed_height is None) and (fixed_aspect is None):
		raise UserError('Must either have a fixed height or a fixed aspect ratio')
	if (fixed_height is not None) and (fixed_aspect is not None):
		raise UserError('Cannot use both fixed height and fixed panel aspect ratio')

	if fixed_height is not None:
		layout = zool.Figure(figwidth=zool.Fixed(width), figheight=zool.Fixed(fixed_height), layout='vertical', padding=padding, **kwargs)
	if fixed_aspect is not None:
		layout = zool.Figure(figwidth=zool.Fixed(width), figheight=zool.FromChildren(), layout='vertical', padding=padding, **kwargs)

	for j in range(nrows):
		rlabel = 'r{:02d}'.format(j+1)
		if fixed_aspect is not None:
			layout.add('base', width=zool.core.FromParent(), height=zool.Named(label_fmt.format(1,1)), layout='horizontal', padding=padding, label=rlabel)
		if fixed_height is not None:
			layout.add('base', width=zool.core.FromParent(), height=zool.Fill(), layout='horizontal', padding=padding, label=rlabel)
		for i in range(ncolumns):
			if fixed_aspect is not None:
				layout.add(rlabel, width=zool.Fill(), height=zool.core.FixedAspect(fixed_aspect), label=label_fmt.format(j+1,i+1))
			if fixed_height is not None:
				layout.add(rlabel, width=zool.Fill(), height=zool.core.FromParent(), label=label_fmt.format(j+1,i+1))
	layout.layout()
	return layout
