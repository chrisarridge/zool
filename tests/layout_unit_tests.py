# # Test a single base with 1cm margins all around and an 8x8 fixed width, should give us a 10x10 figure and x,y=1,1

# # Test a vertical stack with different heights.

# # Test a vertical stack with fixed, fixedaspect, and named on the heights.
# def test_mixedconstraint_vertical_stack(self):
    
#     layout = zool.Layout(figwidth=10.0, figheight=zool.FromChildren(),
#                      layout='vertical', padding=0.5,
#                      margin_top=1, margin_bottom=1, margin_left=1, margin_right=1)
#     layout['a'] = zool.Element(height=5.0)
#     layout['b'] = zool.Element(height=zool.Named('a'))
#     layout['c'] = zool.Element(height=zool.FixedAspect(2))
#     layout['d'] = zool.Element(height=zool.Named('c'))
#     layout['e'] = zool.Element(height=5.0)
#     layout.complete()

#     # Expect heights: a=5.0, b=5.0, c=4.0, d=4.0, e=5.0, a total height of 27.

# # Test a vertical stack with fill constraints on all and a fixed height.


# # Test a vertical stack with a mixture of fixed and fill constraints.


# # unit test the factory functions

# # Test the layout failure with the constraints don't work:
# # - missing ids when using named
# # - conflicting: fill when using from children.



# # Test of kiwisolve core as used inside zool
...
class TestPlotElement(unittest.TestCase):
    def test_base_layout(self):
        """Check that we can set the correct base size without any child items.
        """
        ...
        # Fixed height and set the width from a fixed aspect ratio (width/height).
    ...
    def test_simple_vertical_stack(self):
        ...
        """
        layout = zool.Layout(figwidth=10.0, layout="vertical", padding=0.1,
        ...
        # The total plot height will now be (0.5 + 0.5) + (4 + 4 + 4 + 2) + 3*0.1
        self.assertAlmostEqual(layout["base"].height.value(), (0.5 + 0.5) + (4 + 4 + 4 + 2) + 3*0.1)

    def test_simple_vertical_stack_with_named(self):
        """Check that we can construct a vertical stack where the height of one element comes from another element.
        """
        layout = zool.Layout(figwidth=10.0, layout="vertical", padding=0.1,
                            margin_left=0.5, margin_right=0.5, margin_top=0.5, margin_bottom=0.5)     # omitting figheight means it defaults to from children
        layout["a"] = zool.PlotElement(height=4.0)
        ...
        self.assertAlmostEqual(layout["base"].height.value(), (0.5 + 0.5) + (4 + 2 + 4 + 2) + 3*0.1)


    def test_simple_vertical_stack_with_split(self):
        """Construct a vertical stack where one element is split vertically in to two.
        """
        layout = zool.Layout(figwidth=10.0, layout="vertical", padding=0.1,
                            margin_left=0.5, margin_right=0.5, margin_top=0.5, margin_bottom=0.5)     # omitting figheight means it defaults to from children
        layout["a"] = zool.PlotElement(height=4.0)
        layout["b"] = zool.PlotElement(height=4.0)
        layout["c"] = zool.PlotElement(height=zool.FromChildren(),margin_top=0.5,margin_bottom=0.5)
        layout["d"] = zool.PlotElement(height=2.0)
        ...
        self.assertAlmostEqual(layout["d"].height.value(), 2)
        self.assertAlmostEqual(layout["base"].height.value(), (0.5 + 0.5) + (4 + 4 + 5 + 2) + 3*0.1)

    def test_simple_vertical_stack_filled(self):
        """Construct a vertical stack where the height of each element is obtained by filling the available space.
        """
        layout = zool.Layout(figwidth=10.0, figheight=17.0, layout="vertical", padding=0.25,
                            margin_left=0.5, margin_right=0.5, margin_top=0.5, margin_bottom=0.5)     # omitting figheight means it defaults to from children
        ...
        # The height of the plot is 17, with a margin of 0.5 and the top and bottom and padding of 0.25,
        # so 17 - (0.5 + 0.5) - (4*0.25) = 15 remaining, divided among 5 panels = 3
    ...
    def test_simple_vertical_stack_fixedaspect(self):
        """Test a simple vertical stack where the height of each panel is determined by a fixed aspect ratio.
        """
        layout = zool.Layout(figwidth=10.0, layout="vertical", padding=0.25,
                            margin_left=0.5, margin_right=0.5, margin_top=0.5, margin_bottom=0.5)     # omitting figheight means it defaults to from children
        ...
        # Each panel has an aspect ratio is 2 so with a width of 9 then their heights will be 4.5, the total
        # plot height will be 0.75 + 2*0.5 + 4*4.5 = 19.75
